...
public class VendingStockManager : SingletonBehaviour<VendingStockManager>
{
	[Serializable]
	public class ShelfDictionary : UnitySerializedDictionary<ProductType, StockManager.ShelvesList>
	{
	}
	...
	private void OnProductAdded(Shelf shelf, ProductType type)
	{
		...
		{
			shelfDictionary.Add(type, new StockManager.ShelvesList
			{
		...
		if (!shelf.ParentPlaceable.IsReserved)
		{
			...
			{
				availableShelfDictionary.Add(type, new StockManager.ShelvesList
				{
					shelves = new List<Shelf>()
				});
			}
	...
	private void OnProductRemoved(Shelf shelf, ProductType type)
	{
		...
		if (shelfDictionary[type].shelves.Contains(shelf))
		{
			if (shelf.GetProductCount() == 0)
			{
				...
				{
					MonoBehaviour.print("Removing product type: " + type.ToString() + " from shelf dictionary");
		...
		else if (shelf.GetProductCount() == 0)
		{
			...
			if (shelfDictionary[type].shelves.Count == 0)
			{
				MonoBehaviour.print("Removing2 product type: " + type.ToString() + " from shelf dictionary");
				shelfDictionary.Remove(type);
				outOfStockProducts.Add(type);
			}
		}
	...
	public float GetCustomerCountMultiplier()
	{
		...
		return Mathf.Clamp(totalScore / 12.5f, 1f, 3f);
	}
		...
		else
		{
			for (int j = 0; j < placeable.Shelves.Count; j++)
			{
				...
				if (shelf2.ContainedType != ProductType.NONE)
				{
					...
					{
						availableShelfDictionary.Add(shelf2.ContainedType, new StockManager.ShelvesList
						{
							shelves = new List<Shelf>()
						});
					}
	...
	private float GetBasketBudget()
	{
		...
		float value = UnityEngine.Random.value;
		float num = 0.5f;
		float num2 = ((value < 0.5f) ? UnityEngine.Random.Range(minInclusive, maxInclusive) : ((!(value < 0.8f)) ? UnityEngine.Random.Range(minInclusive3, maxInclusive3) : UnityEngine.Random.Range(minInclusive2, maxInclusive2)));
		return num2 * num;
	}
